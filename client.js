...
//see if can put this in build maps, would make more sense there
let sectorList = ["main", "map_2", "northsea", "deepnorthsea", "deepnorthsea_2" , "deepnorthsea_3", "dungeon_1", 
                  "northsea_dungeon", "deepnorthsea_oubliette", "lighthouse_1", "createmap", "sub_map", "empty_map", "mainland_bay"];
let tileMapSector = localStorage.getItem("tileMapSector");
let script = document.getElementById("map_script");

if (tileMapSector==="main" || tileMapSector==="map_2"){
  console.log("main");
  script.src="./main.js";
  script.onload = mapLoaded;
}
else if (sectorList.includes(tileMapSector)){
  console.log("other map");
  script.src=`./${tileMapSector + ".js"}`;
  script.onload = mapLoaded;
} else {
  console.log("default to main");
  script.src="./main.js";
  script.onload = mapLoaded;
}

//debug stuffss
let masterDebug = false;//needs button that reloads page without mobs and debug true, with mobs debug false
function debugMode(){
...
let muteSound = false;//needs button and global mute
function mute(){
  if (muteSound===true){
    muteSound=false;
    rain_sound.volume=1;
...
var showDrops = true;//remove html button, make a toggle in pause menu
...
var raining = false;
function toggleRain(onoff){//remove html button, figure out storm clouds and fade in
  if (!onoff){
    raining=false;
    playRain();
  } 
...
function toggleStuck(){
  //lmao just kill the player and they respawn XD//rework when death penalties added
...
var DRAWSIZE = 16;
var BLOCKSIZE = 16;
function toggleMap(){//can this be reworked to show bigger maps without quality being terrible?
  if (bigMap===true){
    BLOCKSIZE = 16;
    DRAWSIZE = 16;
    bigMap=false;
...
function drawPauseMenu(){
  if (!showStats){showStats=true;}
  //ctx.clearRect(135,50, 150,200);
  ctx.fillStyle="rgba(139, 69, 19, 0.5)";
  ctx.fillRect(135,50, 165,235);
  ctx.fillStyle="white";
  ctx.font="14px Arial";
  ctx.fillText("MENU", 195, 65);
  ctx.font="8px Arial";
  ctx.fillText("Menu and inventory junk here...", 160, 75);
}
...
function drawStats(){
  if (!showStats){
   drawIcon("statDisp", 4,263);
    ...
    drawIcon("longbow", 4, 168);
    ctx.fillText("archery xp: " + player.skills.archery.xp + "(" + player.skills.archery.lvl + ")", 20, 180);
    drawIcon("raft", 4, 183);
    drawIcon("sail", 4, 183);
    ctx.fillText("sailing xp: " + player.skills.sailing.xp + "(" + player.skills.sailing.lvl + ")", 20, 195);
  }
  //draw hp bar:
...
//xp drops                    B U G G E D                           /////////////////////////////////
let offset = 0;
let dropList = [];//for dropObj put in {"text": text, "amount": null, "coords":[[x1,y1], [x2,y2]],...}
              // drop list gen'd by dropCoords. make all coords same if message to just display in place
...
function drawPlayer(){
  ...
  if (playerX>10){drawPointX=10} else{drawPointX=10}//playerX}//so, drawPoint === 10 right?
  if (playerY>10){drawPointY=10} else{drawPointY=10}//playerY}
  ...
  if (raftMoving){
    ...
    for (let i=0; i<4; i++){
      ctx.drawImage(spriteSheet, baseTiles['rain'][0], baseTiles['rain'][1], 16,16,
        drawPointX*BLOCKSIZE-BLOCKSIZE+raftWake.x+raftWake.offsetX, drawPointY*BLOCKSIZE-BLOCKSIZE+raftWake.y+raftWake.offsetY, 16+raftWake.offsetX,16+raftWake.offsetY)
    }
    ctx.drawImage(spriteSheet, baseTiles['rain'][0], baseTiles['rain'][1], 16,16,
      drawPointX*BLOCKSIZE-BLOCKSIZE+2*(raftWake.x+raftWake.offsetX), drawPointY*BLOCKSIZE-BLOCKSIZE+2*(raftWake.y+raftWake.offsetY), 16+raftWake.offsetX,16+raftWake.offsetY)
  }
}
...
function drawMap(disp_area){
  ...
  for (row in disp_area){
    ...
    for (col in disp_area[row]){
      ...
      let sprtLoc;
      try {
        sprtLoc=gameObjects[tile_map[sprtX][sprtY]['sprite']['name']]['sprite']
      } catch {
        continue;
      }
        ...
        for (object in tile_map[sprtX][sprtY]['objects']){
          ...
          if (tile_map[sprtX][sprtY]['objects'][object] && typeof tile_map[sprtX][sprtY]['objects'][object].id !== 'undefined' && tile_map[sprtX][sprtY]['objects'][object].type==="object"){
            ...
            if (!isSpriteIdInList(game_objects, tile_map[sprtX][sprtY]['objects'][object].id)){
              ...
              continue;//this means object was deleted from game_objects-NOTE this prob not best way to do this
                       //wait doesn't have to continue because of 'empty' space in list? dungeonStairs as an id-less sprite issue
            }
          }
          ...
          //draw npcs
            ...
            if (isObjectInList(npcs, npcid)){
              ...
              //let npcFacing = tile_map[sprtX][sprtY].objects[object].spriteData.facing;
              sprtLoc=tile_map[sprtX][sprtY].objects[object].sprite[npcFacing];
              ctx.drawImage(spriteSheet, sprtLoc[0],sprtLoc[1], 16,16,
                col*BLOCKSIZE, row*BLOCKSIZE, DRAWSIZE,DRAWSIZE);
              //draw health
              if (filterNpcById(npcs, npcid)[0].aggro===true){
                let mobHp = filterNpcById(npcs, npcid)[0].hp;
                let mobMax = filterNpcById(npcs, npcid)[0].maxHp;
                ctx.fillStyle="red";
                ctx.fillRect(col*BLOCKSIZE, row*BLOCKSIZE, 16, 2);
                ctx.fillStyle="rgb(115, 252, 3, 1)";
                ctx.fillRect(col*BLOCKSIZE, row*BLOCKSIZE, (mobHp/mobMax)*16, 2);
              }
              //draw hit splat
              if (filterNpcById(npcs, npcid)[0].gotHit===true){
                console.log("drawing hitsplat!");
                filterNpcById(npcs, npcid)[0].gotHit=false;
                ctx.drawImage(spriteSheet, baseTiles['hitsplat'][0],baseTiles['hitsplat'][1], 16,16,
      ...
      //draw projectiles
      if (tile_map[sprtX][sprtY].projectiles){
        let project;
        for (project in tile_map[sprtX][sprtY]['projectiles']){
          sprtLoc=tile_map[sprtX][sprtY]['projectiles'][project]['sprite'];
          ctx.drawImage(spriteSheet, sprtLoc[0],sprtLoc[1], 16,16,
            col*BLOCKSIZE, row*BLOCKSIZE, DRAWSIZE,DRAWSIZE);
        }
      }
...
function drawProjectiles(disp_area){
...
var game_object_ids = [];//needed for lots of stuff, mainly removing id-less sprites with particuilar properties
var game_objects = [];//npcs and game_object are *separate*

function mapExit(x, y, map_name, type, newX, newY){//can be any sprite
  this.spriteData = JSON.parse(JSON.stringify(gameObjects[type]));//but must have collision to interact
  ...
  this.playerInteract=function(){
    nextTileMapSector(map_name, newX, newY);
  }
}

let playerCrafting = false;//crafting popup
let craftQueue = [];//used by craftingTable
function craftingTable(x, y){//craftItem() in player functions
...
function Sign(x, y, message){//these *could* be reconstituted, but for now just hardcode them in cause maps pita
  ...
  this.playerInteract=function(){
    ...
    this.lastX = playerX;
    this.lastY = playerY;
    //display a message for the player, transparent tan, brown letters, disappears if player moves
    signMessage = message;
    showSign = true;
    this.displayInterval = setInterval(() => {
...
//player Raft
//initialize Raft if player was sailing previous session
...
//animate raft wake when player sailing
let raftWake = {
  x:0,//+/- direction for wake 
  y:0,
  offsetX:0,
  offsetY:2
};
function raftOffset(){
  if (raftWake.offsetX===0){
    raftWake.offsetX=2;
    raftWake.offsetY=0;
  } else {
    raftWake.offsetX=0;
...
//in/out spike trap, iterate over the tile map for a spiky adventure
function Spiketrap(x, y, interval, damage, popupText=null){
  this.spriteData = JSON.parse(JSON.stringify(gameObjects['spikein']));//starts off spike in ground
  this.spriteData.id=generateID();
  this.retainId = JSON.parse(JSON.stringify(this.spriteData.id));//should work?
  this.x=x;
  this.y=y;
  tile_map[this.x][this.y].objects.push(this.spriteData);
  this.interval=interval;
  this.damage=damage;
  this.spikeInterval = null;//in case we want to take it away?
  setTimeout(() => {
    this.spikeInterval=setInterval(() =>{
      if (this.spriteData.name==="spikein"){
        this.spriteData = JSON.parse(JSON.stringify(gameObjects['spikeout']));
        this.spriteData.id=this.retainId;
      } else {
        this.spriteData = JSON.parse(JSON.stringify(gameObjects['spikein']));
        this.spriteData.id=this.retainId;
      }
      //remove from tile objects by id (or name?)
      //add new sprite
      let object;//needs separate fxn that takes sprite ID, much neater
      let count=0;
      for (object in tile_map[this.x][this.y].objects){//this removes spriteData from tile by id, MAKE A FUNCTION YOU FOOL!
      ...
      tile_map[this.x][this.y].objects.push(this.spriteData);
      if (playerX===this.x && playerY===this.y && this.spriteData.name==="spikeout"){
        this.playerInteract();
      }
    },this.interval);
  },1000);
  this.playerInteract = function(){
    if (this.spriteData.name==="spikein"){return;};
    getAttacked(this.damage);
    if (!popupText){
      popMessage("You are impaled by spikes!");
    } else {
      popMessage(popupText);
    }
  }
}

//player needs brasskey to open this door
...
//player can pass through this door but mobs cannot, locked doors convert to this
...
//Treasurechest2 used for giving player scrolls (for now) and other items
...
//for stuff from mobs orrr.........
function Lootbag(_name, x, y, item){//could be solution for player dropping items?
                                   //ex) drop coin, lootbg with 1 coin spawns below player
                                   //drops over a lootbag go in existing lootbag
  //name is from what dropped it, determines what goes in bag (yet to be implemented)
  ...
  tile_map[this.x][this.y].objects.push(this.spriteData);
  this.playerInteract=function(){
    popMessage("Picked up some loot...");
...
//it was really dumb to do it this way, but the lightning times up with the thunder and that's pretty rad
...
document.addEventListener('keydown', (event) => {
  ...
  if (pause){
    //alternate keys and otherwise ignore the later switch
    return;//return to avoid the other keys
  }
...
function downloadArrayAsJSFile(array, filename){//this is broken? workaround -- copy(JSON.stringify(tile_map));
...
let projectiles = [];
let skipDraws = ["woodplate", "stoneplate"];
let directionalProjectiles = ["arrow", "fireball"];//projectiles that direction matters (name+direction makes "arrowup")
function Projectile(name, x, y, direction, distance, speed, damage, fromPlayer=false, passive=false){
  this.x=x;
  this.y=y;
  ...
  this.damage = damage;
  //this.id=generateID();
  if (directionalProjectiles.includes(name)){
    this.spriteData = JSON.parse(JSON.stringify(gameObjects[name+direction]));
  } else {
    this.spriteData = JSON.parse(JSON.stringify(gameObjects[name]));
  }
  this.spriteData.id=generateID();
  this.start=true;
  this.finished = false;
  this.projectileInterval = setInterval(() => {
    //let proj;
    let offset =directionOffset(this.direction);
    try{
      for (let proj in tile_map[this.x-offset.x][this.y-offset.y].projectiles){
        if (tile_map[this.x-offset.x][this.y-offset.y].projectiles[proj].id===this.spriteData.id){
          delete tile_map[this.x-offset.x][this.y-offset.y].projectiles[proj];
        }
      }
    } catch(err){
      //ha-HA
    ...
    this.start=false;
    try {
      if (!tile_map[this.x][this.y].projectiles) {
        //console.log("init projectile list error");
        tile_map[this.x][this.y]["projectiles"] = [];
      }
    } catch (err) {
      // handle the exception, or don't?
    }
    if (this.finished || this.x>98 || this.y>98 || this.x<1 || this.y<1){
      try{
        tile_map[this.x][this.y].projectiles=[];
        for (proj in tile_map[this.x][this.y].projectiles){
          if (tile_map[this.x][this.y].projectiles[proj].id===this.id){
            console.log("worked2!");
          }
        }
      } catch(err){
      ...
      clearInterval(this.projectileInterval);//this seems to clear up dead projectiles, but maybe better/shorter way to do this?
      this.x==null;
      this.y=null;
      this.prevX=null;
      this.prevY=null;
      ...
      //remove self from projectiles list
      delete this;
      return;
    }
    let skipDraw=false;
    try{
      if (this.spriteData.name==="cloud"){
        //check if woodplate or stoneplate 
        for (let obj in tile_map[this.x][this.y].objects){
          if (skipDraws.includes(tile_map[this.x][this.y].objects[obj].name)){
            //console.log("went over plate")
            skipDraw=true;
          }
        }
        if (!skipDraw){
          //console.log("drawing to tile");
          tile_map[this.x][this.y].projectiles.push(this.spriteData);
        }
      } else {
        tile_map[this.x][this.y].projectiles.push(this.spriteData);
      }
    } catch(err){
      //pretty sure what's happening is it's trying to go off the map but this seems to fix it
      console.log(err);
    }
    //increment x or y dpdn direction
    this.prevX=this.x;
    this.prevY=this.y;
    ...
    //check collision, if fromPlayer, do not damage player if run into it for some reason
    if (!passive){
      let proj_collision = projCollision(this.x, this.y);
      switch(proj_collision){
        case "player":
          //hit player
          if (!fromPlayer){
            getAttacked(this.damage);
        ...
        case "npc":
          //hit npc
          if (!fromPlayer){return;};//for now npcs can't shoot each other
          //get npc by id, npc.getAttacked(this.damage);
          let npc;
          for (npc in tile_map[this.x][this.y].objects){
            if (tile_map[this.x][this.y].objects[npc].type==="npc"){
              filterNpcById(npcs, tile_map[this.x][this.y].objects[npc].id)[0].getAttacked(damage);
              incrementSkill("archery", 2);
            }
          }
          this.finished=true;
          break;
        case true:
          //hit object or tile with collision
          this.finished=true;
          break;
        case false:
          break;
      ...
      if (!raining && this.spriteData.name==="cloud"){
        if (this.x===playerX && this.y===playerY){
          toggleRain(true);
          player.lastCloud=Date.now();
        }
      } 
      else if (raining && this.spriteData.name==="cloud"){
        if (this.x===playerX && this.y===playerY){
          player.lastCloud=Date.now();
        }
      }
...
function projCollision(x,y){//can prob use this for stuff other than projectiles
  //check collision of tile itself
  try{
    if (tile_map[x][y].sprite.name!=="water" && tile_map[x][y].sprite.collision){
      return true;
    }
  } catch (error) {
    return;
  }
  //check collision of any objects or npcs on tile
  if (tile_map[x][y].objects.length!==0){
    //npcs first
    let npc;
    for (npc in tile_map[x][y].objects){
      if (tile_map[x][y].objects[npc].type=="npc"){
        return "npc";//maybe if fire arrow/spell, interacts with obj
      }
    }
    //objects second
    let obj;
    for (obj in tile_map[x][y].objects){
      if (tile_map[x][y].objects[obj].collision){
        return true;//maybe if fire arrow/spell, interacts with obj
      }
    }
  }
...
function testWave(){
  ...
  for (i=0;i<30;i++){
    if (i<15){
        new Projectile("whitewave", playerX-30+i, playerY-15+i, "right", 50, 100, 10, false);
    } else {
        new Projectile("whitewave", playerX-i, playerY-15+i, "right", 50, 100, 10, false);
    }
  }
}

function testStorm(){
  let cloud = generateBlob();
  let speed = Math.floor(Math.random()*300+200);
  let dirs = ["up", "down", "left", "right"];
  let dir = dirs[Math.floor(Math.random()*4)];
  for (let coord in cloud){
    projectiles.push(new Projectile("cloud", cloud[coord][0], cloud[coord][1], "right", 75, speed, 0, true, true));
  }
}

let lastRain = Date.now();
let rainChance = 0;
function stormGenerator(){
  if (lastRain+15000<Date.now()){
    lastRain=Date.now();
    let numClouds = Math.floor(Math.random()*Math.floor(rainChance/5));
...
//generates a crappy polygon, good enough
function generateBlob() {
  // Randomly generate the number of vertices for the polygon
  const numVertices = Math.floor(Math.random() * 6) + 4; // Between 4 and 9 vertices

  // Randomly generate the radius of the polygon
  const radius = Math.floor(Math.random() * 10) + 10; // Between 10 and 19 pixels

  // Randomly generate the starting point for the polygon
  ...
  for (let i = 0; i < numVertices; i++) {
    ...
    const x = startX + radius * Math.cos(angle) + (Math.random() * radius * 0.5);
    const y = startY + radius * Math.sin(angle) + (Math.random() * radius * 0.5);
    vertices.push([Math.round(x), Math.round(y)]);
  }

  // Generate the coordinates for the filled-in polygon
  const coordinates = [];
  for (let y = 0; y < 100; y++) {
    for (let x = 0; x < 100; x++) {
  ...
  // Return the array of (x, y) coordinates for the filled-in polygon
  return coordinates;
}

// Helper function to check if a point is inside a polygon (used by generateBlob)
...
function Player(){
  ...
  this.update = function(){
    ...
    if (this.lastCloud+1000<Date.now() && raining){
      popMessage("The sun comes back out...");
      toggleRain(false);
    }
    ...
    for (skill in this.skills){
      if (skill!=='health'){
        checkLevelUp(this.skills[skill]);
      } else {
        checkLevelUp(this.skills['health'], true);
      }
    }
  }
}

function incrementSkill(skill, amount){
  player.skills[skill].xp+=amount;

  if (showDrops && skill!=="walking" && skill!=="sailing"){
    offset+=200;
    if (offset>1000){
      offset = 0;
    }
    setTimeout(() => {//very bugged, not game breaking but simultaneous xp drops throw errors and flicker the drops
      dropList.push({"text":skill, "amount":amount, "coords":dropCoords()});
    },offset);
  }
  localStorage.setItem("playerStats", JSON.stringify(player.skills));
}

//outside function! then maybe mobs could level up?
function checkLevelUp(skill, hp=false){
...
function attack(target){
  let bonus = 0;
  if (player.holding.name!=="nothing"){
    bonus = player.holding.itemObj.attBonus;
  }
  //maybe other worn items have att bonus also, check slot and add to bonus accordingly
  if (playerSailing){//unless fighting water mob
    player.skills.health.health-=Math.floor(Math.random()*25);
    incrementSkill("strength", -10);
    incrementSkill("survival", +5);
    popMessage("You slip and bust your ass in the raft trying to be cute...");
  }
  target.getAttacked(player.skills.strength.lvl + bonus);
}

function getAttacked(damage){
  let dodge = Math.floor(Math.random()*player.skills.defense.lvl);
  if (player.body.name!=="nothing"){
  ...
  if (damage > 0){
    playSound(hit_sound);
    player.skills.health['health']-=damage;
    player.gotHit = true;//checked and turned off in drawPlayer
  } else {
    //player dodges hit
...
function freeze(charObj, duration){
  if (!charObj.frozen){
    //set interval to prevent player movement?
    charObj.frozen = true;
    charObj.frozenTimeout = setTimeout(() => {
      charObj.frozen=false;
      clearTimeout(charObj.frozenTimeout);
    }, duration);
  }
}

//need to figure out way to put this in outside fxn, unnecessary to have all these fxns in player
let usableItems = ['fish', 'cookedfish', 'apple', 'tunafish', 'cookedtuna', 'arrow'];
//equip is really just player use
function equip(){
  let itemObject = player.inventory[player.invPosition].itemObj;//oh wow and even referencing player instead of this.
  //crafting
  if (playerCrafting){
    //put item in craftQueue
    if (craftQueue.length>=20){return;};
    //return return if item already in queue
    let item;
    for (item in craftQueue){
      if (craftQueue[item]===itemObject){
        return;
      }//else it's not in list
    }
  ...
  if (nextTo('stump3') || nextTo('stump2')){//2 & 3 look the same, prob get rid of one lol
    if (player.inventory[player.invPosition].name==="log" && player.inventory[player.invPosition].amt > 0 && player.holding.name==="axe"){
      //split logs, add new or increment in player.inventory, split logs will have to be new item, possibly new sprite, -log, plus 2 split logs
      addToInv("log", -1);
      addToInv("splitlog", 2);
      popMessage("You split a log...");
      return;
    } 
    else if (player.inventory[player.invPosition].name==="splitlog" && player.inventory[player.invPosition].amt > 0 && player.holding.name==="knife"){
      addToInv("splitlog", -1);
      addToInv("arrow", 4);
      popMessage("You whittle some primitive arrows...");
      return;//then primitive "arrow" can be used with arrowhead to make badass arrow?
    }
  }
  ...
  else if (itemObject.equippable===true){
  ...
  else if (usableItems.includes(itemObject.spriteData.name)){
...
let craftableItems = {//test this in separate file cause it could get long
  ...
  "longbow":{"splitlog":4, "hide":1, "string":4}
};//

function craftItem(){
  if (craftQueue.length<=1){return};
  ...
  let itemCheck=Object.keys(craftableItems[currentItem]).length;
  let rawItem;
  for (rawItem in craftQueue){
    if (!Object.keys(craftableItems[currentItem]).includes(craftQueue[rawItem].spriteData.name)){continue};
    ...
    if (player.inventory[invPos].amt >= craftableItems[currentItem][craftQueue[rawItem].spriteData.name]){
      ...
      popMessage(`You don't have enough ${craftQueue[rawItem].spriteData.name}`);
      craftQueue = [];
      return;
    }
  }
  ...
  incrementSkill("crafting", craftingExp);
  craftQueue = [];
    ...
    if (movementInterval === null) {
      movementInterval = setInterval(() => {
        ...
        if (potentialX !== playerX || potentialY !== playerY) {
          if (!playerSailing){
            incrementSkill('walking', 1);
          } else {
            incrementSkill('sailing', 1);//maybe there's other modes of transportation? flying?
          }
        }
...
function interactNPC(nextX, nextY){
  ...
  for (object in tile_map[nextX][nextY].objects){
    if (tile_map[nextX][nextY].objects[object].type=="npc"){
      ...
      if (targetNPC.spriteData.attackable===true && player.lastAttack+player.attSpeed < Date.now()){
        attack(targetNPC);//hmmm?
...
function getDispArea(){//player view, although it coudl be useful for other stuff
  var minX=9;
  var minY=9;
  var dispList=[]
  for (let i=playerY-minY;i<playerY+10;i++){
    let innerList=[]
    for (let j=playerX-minX;j<playerX+10;j++){
      innerList.push([j,i]);
    }
    dispList.push(innerList);
  }
...
function eat(name){
  //get inv pos, if player.inventory[invPos].amt>1, add corresponding hp and subtract 1 from amt
  let invPos = player.inventory.findIndex(obj => obj.name ===name);
  let heals;
  if (player.inventory[invPos].amt > 0){
    ...
    popMessage(`You don't have any ${name}`);
  }
}
...
function useItem(name){
  ...
  switch(name){
    ...
    //other edible cases here
      ...
      return;
    case "arrow":
    //and other arrow/projectile types?
      if (player.holding.name==="longbow" || player.holding.name==="oakbow"){
        if (player.inventory[player.invPosition].name==="arrow" && player.inventory[player.invPosition].amt>0){
          if (player.lastAttack+500 > Date.now()){
            return;
          } else {
            player.lastAttack=Date.now();
          }
          //fire arrow, bow and player archery lvl (and maybe arrow type) determine damage, projectile obj gives xp if hit lands and fromPlayer===true
          addToInv("arrow", -1);
          new Projectile("arrow", playerX, playerY, player.lastDirection, 10, 100, 5, true);
        } else {
          popMessage("Out of ammo!");
        }
      }
  }
}

function addToInv(name, amt=null){
  if (!amt){
    amt=1;
  }//else it is defined
  ...
  if (player.inventory.find(obj => obj.name === name)){
    let invPos = player.inventory.findIndex(obj => obj.name ===name);
    player.inventory[invPos].amt += amt;
  } else {
    player.inventory.push({'name':name, 'amt':amt, "itemObj":{"spriteData":gameObjects[name]}})
  }
}
...
player.skills.health['health']=player.skills.health.max;//remove this?
var ghostR = [48,80];//ghost facing right coords
var ghostL = [64,96];//ghost facing left coords
var ghostFacing='rt';//probably put these in Player and put ghostR/L in base_tiles.js
...
if (playerStats!==null){
  ...
  playerStats=null;
  //add any skills player doesn't have from before updates
  if (!player.skills.health){
    player.skills['health']={"xp":0, "lvl":1, "health":100, "max":100};
  }
  if (!player.skills.playerLvl){
    player.skills.playerLvl=1;
  }
  ...
  if (!player.skills.cooking){
    console.log("setting cooking skill")
    player.skills['cooking']={"xp":0, "lvl":1};
  }
  if (!player.skills.archery){
    player.skills['archery']={"xp":0, "lvl":1};
  }
  if (!player.skills.sailing){
    player.skills['sailing']={"xp":0, "lvl":1};
  }
}
//get inventory from local
var pObjList = {//this is for reconstituting saved items
  "axe":Axe,
...
if (!player.inventory.some(obj => obj.hasOwnProperty("name") && obj["name"]==="knife")){
  player.holding={"name":"nothing", "itemObj":null};
  player.inventory.push({"name":"knife", "itemObj":new Knife})
}
...
//end map building in debug mode/////////////////////////////////////////////
  ...
  this.action = function(x, y){
    ...
    if (tile_map[x][y].sprite.name==='water'){
      ...
      this.isFishing = setTimeout(() =>{
        ...
        if (Math.floor(Math.random()*player.skills.survival.lvl) + Math.floor(Math.random()*player.skills.fishing.lvl) < Math.floor(Math.random()*2)){
        ...
        if (Math.floor(Math.random()*player.skills.fishing.lvl) > Math.floor(Math.random()*4)){
          ...
          incrementSkill('fishing', fishList[caughtFish].xp);//change to be xp of fish caught
...
function Longbow(){
  this.equippable=true;
  this.spriteData = JSON.parse(JSON.stringify(playerObjects['longbow']));
  this.spriteData.id=generateID();
  this.attBonus = 0;
  this.isHeld = false;
  this.onGround = false;
  this.action = function(_x, _y){//if player.holding.name==="longbow" || "oakbow" && has arrows, FIRE
    //no necessary action
    //bow_release.play();
  }
}
...
function Knife(){
  this.equippable=true;
  this.spriteData = JSON.parse(JSON.stringify(playerObjects['knife']));
  this.spriteData.id=generateID();
  this.attBonus=1;
  this.isHeld=false;
  this.onGround=false;
...
function checkCollision(nextX, nextY, npc=false){
  ...
  if (playerSailing && npc===false){//only runs for player
    ...
    if (tile_map[nextX][nextY].sprite.name==='water' && paths===0){
      ...
      localStorage.setItem("raftLoc", JSON.stringify([playerRaft.x, playerRaft.y]));
      if (Math.floor(Math.random()*player.skills.survival.lvl) < Math.floor(Math.random()*3)){
...
function getDirection(mobFacing, mobDirection, prevX, prevY, newX, newY){
  if (prevX !== newX){
    if (prevX < newX){
      mobDirection="left";
      mobFacing="left";
      return {"dir":mobDirection, "face":mobFacing};
    }
    else if (prevX > newX){
      mobDirection="right";
      mobFacing="right";
  ...
  return {"dir":mobDirection, "face":mobFacing};//hmmm
}
...
//if you're wondering why the fuck all these are here, I am too XD need to consolidate those 1 liner fxns
//end utility functions///////////////////////////////////////////////////////////////////////////////////////////////

//passive npc setup testing///////////////////////////////////////////////////////////////////////////////////////////
var move_dir=["up","down","left","right"];

function moveNPC(curX,curY, nextX, nextY){//for passive (non-aggressive/tracking) movement
  //calculate random move, return x,y
  let facing="right";
  let moveDir;
  //facing determined by moveDir determined by curX/Y and nextX/Y
  /*
  if (!definite){
  ...
  */
  ...
  let collision = checkCollision(nextX, nextY, true);
...
//needs to be replaced with A* fxn that anything can use to generate a path
...
function Shopkeep(x, y, _name, greeting, selling={}){//maybe these kind of npcs don't go in npcs list?
                                        //{"item":{"amt":5, "price":5}}
...
let showShop = false;
function shopPopup(_text, _items){//text either manual or comes from Shopkeep properties
...
function Spider(){
  ...
  this.walkingPath = [];
  this.moving=false;
  this.playerLastPos = {x:null, y:null};
  this.direction = "right";
  this.lastCoords = [this.x, this.y]
  this.lastMove = Date.now();
  this.update = function(){
    if (this.lastMove+5000 < Date.now()){
      if (this.lastCoords[0]===this.x && this.lastCoords[1]===this.y){
        //mbo probably stuck or something
        this.moving=false;
        this.walkingPath=[];
        this.lastMove = Date.now();
      }
   ...
   //check if npc dead
    if (this.hp<1){
      incrementSkill("strength", 2);
      incrementSkill("health", 10);
      let loot;
    ...
    if (now > this.lastTime+restTime){
      ...
      if (this.aggro===true && !this.moving){
        this.walkingPath=findPath(this.x, this.y, playerX, playerY);
        //console.log(this.walkingPath);
        this.moving=true;
      }
      else if (this.aggro===false && !this.moving){
        //I don't think this is even doing anything?
        let randTile = getRandomValidTile(this.x, this.y);
        this.walkingPath = findPath(this.x, this.y, randTile[0], randTile[1]);
        this.moving=true;
        /*
        let randX;
        let randY;
        randX = Math.floor(Math.random()*10+this.x);
        randY = Math.floor(Math.random()*10+this.y);
        this.walkingPath = findPath(this.x, this.y, randX, randY);
        this.moving=true;
        */
      }

      let newCoords = [this.x, this.y, "right"];
      if (this.moving===true){
        if (this.walkingPath.length>0){
          if (this.aggro===true){
            if (this.walkingPath[0][0]===playerX && this.walkingPath[0][1]===playerY){
              getAttacked(this.strength);
              this.webPlayer();
              this.walkingPath=[];
              this.moving=false;
            } else {
              let facingDirection = getDirection(this.spriteData.facing, this.direction, this.x, this.y, this.walkingPath[0][0], this.walkingPath[0][1]);
              this.spriteData.facing = facingDirection["face"];
              this.direction = facingDirection["dir"];//figure out why it wouldn't change by passing to getDirection?
              newCoords = moveNPC(this.x, this.y, this.walkingPath[0][0], this.walkingPath[0][1]);
      ...
      //remove from old tile, put back on new tile
      tile_map[this.x][this.y].objects = filterObjById(tile_map[this.x][this.y].objects, this.spriteData.id);
      this.x=newCoords[0];
      this.y=newCoords[1];
      this.facing=newCoords[2];
      tile_map[this.x][this.y].objects.push(this.spriteData)


      if (this.aggro===true){ 
        if (this.playerLastPos.x!==playerX || this.playerLastPos.y!==playerY){
          this.moving=false;
          this.walkingPath=[];
        }
      }
...
function Rat(){
  ...
  this.gotHit = false;
  this.playerLastPos = {x:null, y:null};
  this.walkingPath = [];//list of coords. walk them until empty or player moves (for now)
  this.trackingToPlayer=false;
  this.trackToOrigin=false;
  this.trackToRandom=false;
  this.moving=false;
  this.started=false;//to fix not moving until aggroed then unaggroed (not sure lol)
  this.lastCoords = [this.x, this.y]
  this.lastMove = Date.now();
  this.update = function(){
    if (this.lastMove+5000 < Date.now()){
...
//spriteData derived from name
//name, hp, delay, strength, xp, 
function Skeleton(){
...
function getRandomValidTile(mobX, mobY){
  let validTiles = [];
  let potentialTiles = getPathArea(mobX, mobY);
  for (let x in potentialTiles){
    for (let y in potentialTiles[x]){
      let xCoord = potentialTiles[x][y][0];
      let yCoord = potentialTiles[x][y][1]; 
      if (tile_map[xCoord][yCoord].sprite.collision===true){
        continue;
      }
      let coll = false;
      let obj;
      for (obj in tile_map[xCoord][yCoord].objects){
        if (tile_map[xCoord][yCoord].objects[obj].collision===true){
          coll = true;
          break;
        }
      }
      if (coll===false){
        validTiles.push(potentialTiles[x][y]);
      }
    }
  }
  //let changeChance = [1,2][Math.floor(Math.random()*2)];
  /*
  let randTile = validTiles[Math.floor(Math.random()*validTiles.length)];
  if (Math.random()*10 > 0){
    randTile = validTiles[0];
    //console.log(`default to 0: ${validTiles[0]}`)
  }
  randTile=validTiles[0];
  */
  let randTile;
  try{
    randTile = getValidSpawnCoord(mobX-10, mobY-10, mobX+10, mobY+10);
    randTile = [randTile.y, randTile.x];
  } catch(err){
    randTile=[mobX, mobY];
  }
  return randTile;
...
//too many parameters... how to better do this?
function Rangemob(name, projType, projSpeed, projDist, hp, str, delay, retreatMoves, retreatDelay, xp) {
                  //this should be like a list or dict so the mob is just a data set and gets put as the arg
    this.name = name; //its in spriteData but had to do it this way for some reason
    this.hp = hp; //lel just testing
    this.maxHp = hp;
    this.str = str;
    this.spriteData = JSON.parse(JSON.stringify(gameObjects[this.name]));
    this.spriteData.id = null;
    ...
    this.strength = 5;
    this.lastDirection = "right";
    this.retreatTimeout = null;
    this.retreating = false;
    this.retreatCount = retreatMoves;
    ...
    this.update = function () {
      if (this.hp < 1) {
        if (Math.floor(Math.random() * 100 > 90)) {
          game_objects.push(new Lootbag(name, this.x, this.y, "arrow"));
        } else {
          game_objects.push(new Lootbag(name, this.x, this.y, "coin"));
        }
        incrementSkill("strength", xp.strengthXp); //what if something else kills it though?
        incrementSkill("health", xp.healthXp);
        playSound(skele_die_sound);
        removeNPC(this.spriteData.id, this.x, this.y);
      }
      let now = Date.now();
      let restTime = this.delay; //?

      if (now > this.lastTime + restTime) {
        this.lastTime = now;
        //check distance to player
        if (distToPlayer(this.x, this.y) <= 8) {
          this.retreatTimeout = setTimeout(() => {
            //gnoll is probably attacking player, runs away after a second
            this.retreating = true;
            this.delay = this.retreatDelay;
            //clearInterval(this.retreatInterval);
            this.retreatTimeout = null;
          }, this.delay);
          let shootDirection = "right"; //just default in case something weird happens
          let shoot = false;
          //this gonna be ugly but should work - in lieu of actually using lastDirection. would entail changing moveNPC to return a 4th thingy :/
          if (playerY === this.y) { //on the same horizon
            //left or right shot
            if (playerX < this.x) {
              //shoot left
              shootDirection = "left";
              this.spriteData.facing = "left";
              shoot = true;
            } else {
              //shoot right
              this.spriteData.facing = "right";
              shootDirection = "right";
              shoot = true;
            }
          }
          else if (playerX === this.x) { //on the same vertice
            //up or down 
            if (playerY < this.y) {
              //shoot up
              shootDirection = "up";
          ...
          if (shoot){
            if (this.lastShot+1250<Date.now()){
              new Projectile(projType, this.x, this.y, shootDirection, this.projDist, this.projSpeed, this.strength, false);
              this.lastShot=Date.now();
            }
            shoot = false;
          }
...
// Rangemob(name, projType, projSpeed, projDist, hp, str, delay, retreatMoves, retreatDelay)

let complexMobInfo = {
  "gnoll":["gnoll", "pebble", 100, 8, 100, 8, 2000, 5, 500, {strengthXp:5, healthXp:10}],
  "rangeGoblin":["rangeGoblin", "arrow", 100, 10, 150, 20, 1000, 10, 200, {strengthXp:10, healthXp:15}],
  "mageLich":["mageLich", "fireball", 100, 12, 300, 35, 1000, 8, 200, {strengthXp:20, healthXp:20}]
}
...
const generatorList = {
  ...
  "rat":Rat, 
  "skeleton":Skeleton, 
  "gnoll":Rangemob,//complex mob
  "rangeGoblin":Rangemob,//complex mob
  "mageLich":Rangemob//complex mob
}
...
function generateNPC(name, domain, amt, interval, xMin, xMax, yMin, yMax){
  ...
  let map = localStorage.getItem("tileMapSector");
  genInterval = setInterval(function(){
    if (localStorage.getItem("tileMapSector") !== map){
    ...
    if (countObjectsByPropertyValue(npcs, "domain", domain) < amt){
      ...
      let newNPC;
      try {
        newNPC = new generatorList[name];
      } catch {
        newNPC = new generatorList[name](name, 
          complexMobInfo[name][1],
          complexMobInfo[name][2], 
          complexMobInfo[name][3], 
          complexMobInfo[name][4], 
          complexMobInfo[name][5], 
          complexMobInfo[name][6], 
          complexMobInfo[name][7], 
          complexMobInfo[name][8],
          complexMobInfo[name][9]
        );
      }
...
function update(){
  ...
  if (!death){
    ...
    stormGenerator();//produces storm clouds every 15 seconds
    updateNPCs();
    player.update();
    if (!bigMap){
...
animationInterval = setInterval(() => {
  ctx.clearRect(0,0,0,304);
  ctx.fillText(`${loadText}`, 100,150);
  loadText+=".";
  animationCount++;
  if (animationCount>=8){
